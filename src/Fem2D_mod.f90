...
MODULE OMP_PARALLEL
    ...
    subroutine set_num_threads
        ...
        success = SETENVQQ(FN)
        fn= 'OMP_NUM_THREADS='//STR_ITER_TMP
        success = SETENVQQ(FN)
        ! print*, success

        call get_environment_variable("MKL_NUM_THREADS", homedir)
        ! print*, homedir
        call get_environment_variable("OMP_NUM_THREADS", homedir)
        ! print*, homedir
        ! pause
...
MODULE GLOBAL_ARRAYS_MODULE
    ...
    Subroutine Allocate_Global_Arrays(L, NODTOL_,NEQ_f_) 
        Implicit None 
        Logical, Intent(In)           :: L
        Integer, Intent(In), Optional :: NODTOL_, NEQ_f_
...
MODULE CSR_STORAGE
    ...
    SUBROUTINE CSR_MAIN(ICHOICE)
        ...
        USE ELEMENTS_MODULE,    only: NEQ_f, NBF_2d, NEL_2d, NUNKNOWNS_f
    ...
    SUBROUTINE CSR_NUMBERING
        ...
        USE ENUMERATION_MODULE, only: NsuN1, NsuN2, GNTR

        IMPLICIT NONE

        INTEGER :: I, II, J, JJ, KK, L, MZ
...
Module DirichletBoundaries
    ...
    Subroutine updateAllNodesOfTheBoundary( FemValueName, bnd_elements, bnd_faces, procedure_)
        Implicit None 
        !<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
        ! Arguments
        !<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
