Module ExtraEquations
 ...
 Use PHYSICAL_MODULE,         Only: Pambient_o_Pchar, ratio_of_pressures, position
 use basis_calculations,      only: BASIS_2d



  Private
  Public :: CentroidOfTheBubble, DragForceCalculationNewtonian, SurfaceIntegration, &
                                 int_Z_dV
...
Contains
    ...
    Function DragForceCalculationNewtonian( NELEM, NED ) Result(DragForce)
        implicit none
        !<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>   
        !  ARGUMENT
        !<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>   
        ...
        call copyArrayToLocalValues( TL(:,getVariableId("Trr")),  nm_mesh(nelem,:), Srr_loc )
        call copyArrayToLocalValues( TL(:,getVariableId("Trz")),  nm_mesh(nelem,:), Srz_loc )
        call copyArrayToLocalValues( TL(:,getVariableId("Tzz")),  nm_mesh(nelem,:), Szz_loc )
        call copyArrayToLocalValues( TL(:,getVariableId("Ttt")),  nm_mesh(nelem,:), Stt_loc )
    ...
    end Function DragForceCalculationNewtonian
