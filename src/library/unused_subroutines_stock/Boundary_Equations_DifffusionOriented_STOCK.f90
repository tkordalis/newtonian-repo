...
! *****************************************************************
! *****************************************************************
!  Divergence theorem to the convective term of the concentration
! *****************************************************************

    Subroutine openConcentrationFlux( NELEM, NED, TEMP_TL, TEMP_RES, STORE )
        Use VariableMapping
        Use PHYSICAL_MODULE
        Use ELEMENTS_MODULE,         Only: NBF_2d,  NEQ_f, NUNKNOWNS_f
        Use GAUSS_MODULE,            Only: WO_1d, NGAUSS_1d, &
        ...
        LOOP_GAUSS: DO KK = 1, NGAUSS_1d
            ...
            loop_residuals_f:DO IW = 1, NBF_2d
                    ...
                    TERM_RES     = 0.D0
                    TERM_RES(getVariableId("C"))  = ( SBFN * ( nR * (Vr-dRdt) + nZ * (Vz-dZdt) ) * C - BIFN *(nR*dCdR + nZ*dCdZ)/PeN ) * R
    ...
    end Subroutine openConcentrationFlux
    ...
    Subroutine weakHenry( NELEM, NED, TEMP_TL, TEMP_RES, STORE, gVar )
        ...
        LOOP_GAUSS: DO KK = 1, NGAUSS_1d
            ...
            loop_residuals_f:DO IW = 1, NBF_2d
                    ...
                    TERM_RES     = 0.D0
                    ! TERM_RES(getVariableId("C"))  = SBFN * ( nR * (Vr-dRdt) + nZ * (Vz-dZdt) ) * KoN * gVar * R/PeN
...
! *****************************************************************
    ...
    Subroutine weakConcentrationFluxUbubble( NELEM, NED, TEMP_TL, TEMP_RES, STORE, bmol, bvolume, bvelocity )
        Use VariableMapping
        Use PHYSICAL_MODULE
        Use ELEMENTS_MODULE,         Only: NBF_2d,  NEQ_f, NUNKNOWNS_f
        Use GAUSS_MODULE,            Only: WO_1d, NGAUSS_1d, &
        ...
        LOOP_GAUSS: DO KK = 1, NGAUSS_1d
            ...
            loop_residuals_f:DO IW = 1, NBF_2d
                    ...
                    ! TERM_RES(getVariableId("C"))  = SBFN * ( nR * (Vr-dRdt) + nZ * (Vz-dZdt) ) * KoN * gVar * R/PeN
                    ! TERM_RES(getVariableId("C"))  = ( SBFN * ( nR * (Vr-dRdt) + nZ * (Vz-dZdt) ) * KoN * gVar - BIFN *(nR*dCdR + nZ*dCdZ)/PeN ) * R
                    TERM_RES(getVariableId("C"))  = SBFN * ( nZ * (bvelocity - dZdt ) + nR * (- dRdt ) )*bmol/bvolume * R
                    !      FORM THE WORKING RESIDUAL VECTOR IN ELEMENT NELEM
                TEMP_RES(IW,1:NEQ_f) = TEMP_RES(IW,1:NEQ_f) + TERM_RES(1:NEQ_f)* WO_1d(KK)  * dS
    ...
    end Subroutine weakConcentrationFluxUbubble
