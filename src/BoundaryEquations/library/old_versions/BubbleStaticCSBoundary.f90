...
Module BubbleStaticCSBoundary
    ...
    Public :: BubbleStaticCS, NewBubbleStaticCS

    Type BubbleStaticCS 
        ! BubbleStaticCS Properties
        Real(8)                            :: Zcenter_o
    ...
    !<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
    Function NewBubbleStaticCS( elements, faces) Result(This)
        Implicit None 
        Integer, Dimension(:), Intent(In) :: elements
        Integer, Dimension(:), Intent(In) :: faces

        Type(BubbleStaticCS)                      :: This

        
        if ( size(elements) /= size(faces) ) Then
            Print*, "[Error] NewBubbleStaticCS."
        ...
        This%faces    = faces
    End Function NewBubbleStaticCS
    ...
    Subroutine applyBoundaryConditions(This, FlagNR, kinematic_logical)
        ...
        Implicit None 
        Class(BubbleStaticCS)   , Intent(In)      :: This
...
End Module BubbleStaticCSBoundary
